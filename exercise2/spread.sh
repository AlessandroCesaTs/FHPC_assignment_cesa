...
do
	for j in {1..5}
	do
		./sgemm_mkl.x $i $i $i >> results/sgemm_mlk_spread.csv
		./dgemm_mkl.x $i $i $i >> results/dgemm_mlk_spread.csv
		./sgemm_oblas.x $i $i $i >> results/sgemm_oblas_spread.csv
		./dgemm_oblas.x $i $i $i >> results/dgemm_oblas_spread.csv
	done
done
